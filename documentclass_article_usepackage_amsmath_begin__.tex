...
\\\\\\\\
( Per LUCA: Scrivo qua dei dubbi con le mie risposte & considerazioni che potrebbero risultare utili prima di trattare la parte dei test numerici. Spero di essermi espressa in maniera abbastanza chiara.
...
ed $\eta$ risultava essere uguale a $\eta$ = - B$(\theta_s - \alpha)_s$. Però qua la situazione è leggermente diversa, perchè le cose sono molto più semplificata grazie all'equilibro imposto.
...
\\\\
In Jülicher abbiamo $\eta$ = - B$(\theta_s - \alpha)_s$, che è l'equazione che mi da $\eta$, ovvero la componente della forza lungo la direzione normale ($"\mathbf{b}"$), ed è derivata rispetto ad s per il motivo che ho elencato sopra.
\\\\
Noi abbiamo $\hat{M}$ = C($\theta$_s - $\alpha$), che è l'equazione lungo M e non è derivata rispetto ad s.
\\\\
Le due equazioni hanno natura diversa.}
\\\\
Ciò nonnostante, se le cose sono state fatte bene, una volta che non si considerano più i termini inerziali nel problema vincolato (Section 5.1),  precisamente tolti $\rho S \mathbf{r}_{tt}$ e $\rho I \theta_{tt}$ nella riga 69, e nelle matrici che seguono, si dovrebbe trovare il vecchio sistema che avevamo in Jülicher. Giusto? )
