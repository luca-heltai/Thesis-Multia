...
$\bullet$\empth{The Lagrangian $\mathcal{L}$}. We define the kinetic energy density 
...
\]
where $\^{N}$,$\^{H}$,$\^{M}$ are given functions on (0,$\infty$)x$\mathbb{R}$x$\mathbb{R}$ and where $\tilde{A}$, $\tilde{B}$, $\tilde{C}$ are positive constants.
\\
...
\]
$\bullet$\empth{The Virtual Word $\delta$W}.Along with the elastic potential we introduce the internal energy dissipation density  and the dissipative force associated as 
