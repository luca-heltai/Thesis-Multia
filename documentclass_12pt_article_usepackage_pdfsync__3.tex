...
We observe that the dynamic equations of motion have the same structure in the two different models. After adding to the equations of motion the three constitutive equations that model the viscoelasticity of the rod in the first model, and the equations of the constrains in second model, we notice that this two different treatments yield to two different final matrix formulations.
\\\\
In Section 3, we after some theoretical results on Hopf bifurcation, our main goal is to observe the same bifurcation in absence of mass, and in absence of viscosity.
\\\\
