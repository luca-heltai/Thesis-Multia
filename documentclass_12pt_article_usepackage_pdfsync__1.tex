...
Under the action of forces, the elastic rod-like filament suffers deformations, which we assume to be planar. Whenever these deformations occur, it will be a contact force \mathbf{n}(s,t) exerted by the material of [s,1] on that of [0,s). \mathbf{n}(s,t) is assumed to lie to the (\mathbf{e_1,e_2})-plane, so that it has the form
...
\]
where \hat{N},\hat{H},\hat{M}are given functions on (0, \(\infty\) ) \times \mathbb{R} \times \mathbb{R}
We model this by introducing the elastic potential density  as
\[ \mathfrak{U}(\mathbf{y}) = \frac{\cA}{2} (\nu - \nu_0)^2
